...
backtitle="Cloud Sync (https://github.com/alexandershalin/rclone_script)"
...
function doUpSync ()
{
	...
	rclone copy ~/RetroPie/saves/ retropie:${remotebasedir}/ --update --skip-links --exclude "readme.txt" --verbose 2>&1 | tee -a ${tmpfile}
...
function doDownSync ()
{
	...
	rclone copy retropie:${remotebasedir}/ ~/RetroPie/saves/ --update --skip-links --exclude "readme.txt" --verbose 2>&1 | tee -a ${tmpfile}
